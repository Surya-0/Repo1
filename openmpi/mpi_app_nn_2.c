...
// Calculate the accuracy of the perceptron model on the given dataset
float calculate_accuracy(Perceptron* perceptron, TrainingData* data, int data_size) {
    int correct_predictions = 0;
    int i;

    for (i = 0; i < data_size; i++) {
        float output = activation_function(perceptron->weight * data[i].input + perceptron->bias);
        if (output == data[i].expected_output) {
            correct_predictions++;
        }
    }
...
Perceptron train_perceptron(TrainingData* data, int data_size) {
    ...
    // Print the trained perceptron's weight, bias and accuracy
    float accuracy = calculate_accuracy(&perceptron, data, data_size);
    printf("Trained perceptron: weight = %f, bias = %f, accuracy = %f\n", perceptron.weight, perceptron.bias, accuracy);
...
int main(int argc, char* argv[]) {
    int rank, size, i;
    TrainingData* data;
    int data_size = 1000; // Assume we have 1,000,000 data points
    ...
    if (rank == 0) {
        ...
        // Print the combined perceptron's weight, bias and accuracy
        float accuracy = calculate_accuracy(&perceptron, data, data_size);
        printf("Combined perceptron: weight = %f, bias = %f, accuracy = %f\n", perceptron.weight, perceptron.bias, accuracy);
    }
